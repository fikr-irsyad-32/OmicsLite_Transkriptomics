# Dataset: GSE203024 (Cancer & pre-cancer vs control)
...
gset <- getGEO("GSE203024", GSEMatrix = TRUE, AnnotGPL = TRUE)[[1]]
...
dx <- pData(gset)[["cancer dx:ch1"]]

pData(gset)$CancerStat <- factor (ifelse(dx == "no known cancer", "Control", "Cancer"))

## Bikin kolom baru bernama 'CancerStat'

group_info <- pData(gset)[["CancerStat"]]

## group_info hanya ambil data metadata sampel kolom ‘CancerStat’ dari gset 
...
volcano_data$Gene[rownames(volcano_data) == "21916"] <- "GABPB1-AS1"

volcano_data$status <- "NO"
volcano_data$status[volcano_data$logFC > 0.585 & volcano_data$adj.P.Val < 0.01] <- "UP"
volcano_data$status[volcano_data$logFC < -0.585 & volcano_data$adj.P.Val < 0.01] <- "DOWN"

ggplot(volcano_data, aes(x = logFC, y = -log10(adj.P.Val), color = status)) +
  geom_point(alpha = 0.6) +
  scale_color_manual(values = c("DOWN" = "blue", "NO" = "grey", "UP" = "red")) +
  geom_vline(xintercept = c(-0.322, 0.322), linetype = "dashed") +
  geom_hline(yintercept = -log10(0.01), linetype = "dashed") +
  theme_minimal() +
  ggtitle("Volcano Plot of DEG in Cancer") +
  geom_text_repel(
    data = subset(volcano_data, status != "NO"),
    aes(label = Gene),
    size = 3,
    show.legend = FALSE
  ) +
...
pheatmap(
  ...
  main = "Top 50 Differentially Expressed Genes in Cancer"
)

# SAVE
write.csv(topTableResults, "GSE203024_DEG.csv")